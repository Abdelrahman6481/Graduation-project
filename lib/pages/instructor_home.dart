...
class _InstructorHomePageState extends State<InstructorHomePage>
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              _buildResultsTable(course, students),
            ],
          ),
        );
      },
  ...
  Widget _buildResultsTable(
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        // Add publish button for the course
        Row(
          mainAxisAlignment: MainAxisAlignment.end,
          children: [
            ElevatedButton.icon(
              icon: const Icon(Icons.publish),
              label: const Text('Publish All Results'),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.red.shade900,
                foregroundColor: Colors.white,
              ),
              onPressed: () => _showPublishResultsConfirmation(course),
            ),
          ],
        ),
        const SizedBox(height: 12),
        ...
        Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return FutureBuilder<Map<String, dynamic>?>(
                future: FirestoreService().getStudentResult(
                  studentId,
                  int.tryParse(course['id'].toString()) ?? 0,
                ),
                builder: (context, snapshot) {
                  ...
                  if (snapshot.connectionState == ConnectionState.done &&
                      result != null) {
                    midterm =
                        (result['midtermGrade'] as num?)?.toDouble() ?? 0.0;
                    final_exam =
                        (result['finalGrade'] as num?)?.toDouble() ?? 0.0;
                    assignments =
                        (result['assignmentsGrade'] as num?)?.toDouble() ?? 0.0;
                    participation =
                        (result['participationGrade'] as num?)?.toDouble() ??
                        0.0;
                    total = (result['totalGrade'] as num?)?.toDouble() ?? 0.0;
                  ...
                  return Container(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          flex: 3,
                          child: Text('${student['name']} (ID: $studentId)'),
                        ),
                        Expanded(
                          flex: 2,
                          child: Text(
  ...
  void _showEditResultsDialog(
    Map<String, dynamic> course,
    Map<String, dynamic> student,
    Map<String, dynamic>? existingResult,
  ) {
    // Default values
    double midterm = existingResult?['midtermGrade'] ?? 0.0;
    double finalExam = existingResult?['finalGrade'] ?? 0.0;
    double assignments = existingResult?['assignmentsGrade'] ?? 0.0;
    double participation = existingResult?['participationGrade'] ?? 0.0;
    String notes = existingResult?['notes'] ?? '';

    // Define semester options
    final List<String> semesterOptions = ['Fall', 'Spring', 'Summer'];
    String selectedSemester =
        existingResult?['semester'] ?? _getCurrentSemester();
    if (!semesterOptions.contains(selectedSemester)) {
      selectedSemester = semesterOptions.first;
    }

    // Generate academic year options (current year and 4 years before)
    final int currentYear = DateTime.now().year;
    final List<int> academicYearOptions = List.generate(
      5,
      (index) => currentYear - index,
    );
    int selectedAcademicYear =
        existingResult?['academicYear'] ?? _getCurrentAcademicYear();
    if (!academicYearOptions.contains(selectedAcademicYear)) {
      selectedAcademicYear = academicYearOptions.first;
    }

    bool publishImmediately = false;

    // Create a variable to store the BuildContext for the loading dialog
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            return AlertDialog(
              title: Text('Edit Results for ${student['name']}'),
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    // Academic period with dropdowns
                    Row(
                      children: [
                        Expanded(
                          child: Column(
                              ...
                              Container(
                                ...
                                child: DropdownButtonHideUnderline(
                                  child: DropdownButton<int>(
                                    ...
                                    items:
                                        academicYearOptions.map((int year) {
                      ...
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            onChanged: (value) {
                              setState(() {
                                midterm = double.tryParse(value) ?? 0.0;
                              });
                            },
                          ),
                        ),
                      ...
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            decoration: const InputDecoration(
                              labelText: 'Assignments (20%)',
                              border: OutlineInputBorder(),
                            ),
                            keyboardType: TextInputType.number,
                            onChanged: (value) {
                              setState(() {
                                assignments = double.tryParse(value) ?? 0.0;
                              });
                            },
                          ),
                        ),
                        ...
                        Expanded(
                          child: TextField(
                            ...
                            decoration: const InputDecoration(
                              labelText: 'Participation (10%)',
                              border: OutlineInputBorder(),
                            ),
                            keyboardType: TextInputType.number,
                            onChanged: (value) {
                              setState(() {
                                participation = double.tryParse(value) ?? 0.0;
                              });
                            },
                          ),
                        ),
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            '${total.toStringAsFixed(1)} (Grade: $letterGrade)',
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
                              color: _getGradeColor(letterGrade),
                            ),
                    ...
                    // Publish checkbox
                    CheckboxListTile(
                      title: const Text('Publish result immediately'),
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    // First check if still mounted before showing loading indicator
                    if (!mounted) return;

                    // Capture the context before any async operations
                    final dialogBuilderContext = context;
                    ...
                    try {
                      ...
                      final courseId =
                          int.tryParse(course['id'].toString()) ?? 0;
                      await FirestoreService().publishStudentResults(
                        courseId,
                        publishAll: true,
                      );
                      ...
                      if (mounted) {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text('All results published successfully'),
                            backgroundColor: Colors.green,
                          ),
                        );
                        ...
                        setState(() {
                          _selectedTabIndex = 2; // Results tab
                        });
                      }
                    } catch (e) {
                      if (mounted) {
  ...
  int _getCurrentAcademicYear() {
    final now = DateTime.now();
    // If we're in the fall semester, the academic year is this year
    // Otherwise it's the previous year (e.g., Spring 2023 is part of 2022-2023 academic year)
    if (now.month >= 9) {
      return now.year;
    } else {
      return now.year - 1;
  ...
  // Add a button to publish all results for a course
  void _showPublishResultsConfirmation(Map<String, dynamic> course) {
    // Get current semester and year for display
    final currentSemester = _getCurrentSemester();
    final currentYear = _getCurrentAcademicYear();
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  'Are you sure you want to publish all results for ${course['name']}?\n\n'
                  'This will make all grades visible to students.',
                ),
                const SizedBox(height: 16),
                Container(
                  padding: const EdgeInsets.all(12),
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text(
                        'Results will be displayed to students as:',
                        style: TextStyle(fontWeight: FontWeight.bold),
                      ),
                      const SizedBox(height: 8),
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  // First check if still mounted before using context
                  if (!mounted) return;
